...
def test_simple_fp8_export(request, qcdq_export_fn):
...
@jit_disabled_for_export()
def test_fp8_export_activation(request, qcdq_export_fn):
    if torch_version < version.parse('2.1.0'):
        pytest.skip(f"OCP FP8 types not supported by {torch_version}")
...
@jit_disabled_for_export()
def test_fp8_export_export_activation(request, qcdq_export_fn):
    if torch_version < version.parse('2.1.0'):
        pytest.skip(f"OCP FP8 types not supported by {torch_version}")
