...
def test_expected_acceptance_rate_uses_original_top_k_without_renormalization():
    ...
    expected = torch.minimum(reference_p[reference_top_k],
    ...
    assert quantized_metrics.expected_acceptance_rate == pytest.approx(expected)
...
def test_identical_models_return_reference_top_k_mass():
    ...
    assert quantized_metrics.expected_acceptance_rate == pytest.approx(expected)
    assert quantized_metrics.expected_acceptance_rate < 1.0
