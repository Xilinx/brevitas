...
"""Exercise every quantizer in WEIGHT_QUANT_MAP / INPUT_QUANT_MAP on a tiny LLaMA.

For every valid combination of keys found in ``WEIGHT_QUANT_MAP`` and
``INPUT_QUANT_MAP`` (defined in ``brevitas_examples/common/generative/quantize.py``),
we quantize a tiny LLaMA using the *same flow as the LLM entrypoint*:
...
# Small group size so per_group combinations exercise real grouping (rather than
# collapsing to a single group when group_size == hidden_size, which could mask errors).
GROUP_SIZE = 8
WEIGHT_BIT_WIDTH = 8
INPUT_BIT_WIDTH = 8


def _entrypoint_arg_defaults():
    """Return the LLM entrypoint's default args (as a Namespace).

    Sourcing defaults from the real arg parser guarantees the test stays in sync
    with the entrypoint instead of hardcoding values that can drift.
    """
    return create_args_parser().parse_args([])


def _flatten_paths(d, depth, prefix=()):
...
def _weight_combinations():
    """Return (id, weight_kwargs) for every WEIGHT_QUANT_MAP leaf."""
    combos = []
    # WEIGHT_QUANT_MAP[fmt][scale_precision][param_method][granularity][quant_type]
...
def _input_combinations():
    """Return (id, input_kwargs) for every INPUT_QUANT_MAP leaf.

    Two path shapes exist:
      * regular:  [fmt][scale_type][scale_precision][param_method][granularity][quant_type]
      * no_scale: [fmt][scale_type][quant_type]
    """
    combos = []
...
def tiny_llama():
    """Load the tiny LLaMA model and a small input batch once per module."""
...
def _generate_quantizers_kwargs(
    ...
    non-None value, exactly as generate_quantizers is called in main.py) and then
    override only the keys under test.
    """
