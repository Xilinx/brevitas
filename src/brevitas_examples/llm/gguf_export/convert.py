...
class ModelBase:
    ...
    def tensor_force_quant(
            self, name: str, new_name: str, bid: int | None,
        ...
        if suffix in name:
            ...
            try:
                ...
                if hasattr(module, "weight_quant") and module.weight_quant.is_quant_enabled:
                    ...
                    scale = module.quant_weight().scale_

                    if isinstance(scale, torch.Tensor):
                        scale = scale.numpy()
                    zp = None  # module.weight_quant.zero_point()
                    # if torch.any(zp != 0):
                    #     zp = zp.numpy()
                    # else:
                    #     zp = None
...
class BaichuanModel(TextModel):
    ...
    def _reverse_hf_permute(
            self, weights: Tensor, n_head: int, n_kv_head: int | None = None) -> Tensor:
