...
options:
  ...
  --attn-quant-format ATTN_QUANT_FORMAT
                        Attention quantization type. Either int or eXmY, with
                        X+Y==weight_bit_width-1. It's possible to add
                        float_ocp_ or float_fnuz_ before the exponent/mantissa
                        bitwidth. Default: None (same as input).
  --attn-param-method {stats,mse}
                        How scales/zero-point are determined. Default: None
                        (same as input).
  --attn-scale-precision {float_scale,po2_scale}
                        Whether input scale is a float value or a po2.
  ...
  --bos-preprocessing {None,document,sequence}
                        Type of BOS token pre-processing for training and
                        evaluation datasets. Default None)
  --few-shot-limit FEW_SHOT_LIMIT
