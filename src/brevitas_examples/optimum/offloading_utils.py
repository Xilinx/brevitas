...
def infer_auto_device_map(
    ...
    while len(modules_to_treat) > 0:
        ...
        if current_max_size is not None and current_memory_used + module_size > current_max_size:
            # Split or not split?
            modules_children = [] if isinstance(module, nn.Parameter) else list(
...
def infer_fx_auto_device_map(
    ...
    for node in model.graph.nodes:
        ...
        if node.op == 'call_function':
            ...
            old_func_mapping[node] = node.target
            node.target = new_func
            # InsertModuleCallAfter
            # if node.target in model.state_dict().keys():
            #     name_minus_attr = '.'.join(node.target.split('.')[:-1])
            #     current_memory_used += module_sizes[name]
            #     device_map[name_minus_attr] = devices[current_device]
    model.recompile()
    model.graph.lint()
    # Direct submodules and parameters
    modules_to_treat = (
        list(model.named_parameters(recurse=False)) + children +
        list(model.named_buffers(recurse=False)))
    # Initialize maximum largest layer, to know which space to keep in memory
    ...
    return device_map, old_func_mapping
