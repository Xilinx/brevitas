...
def get_hadK(n, transpose=False):
    ...
    # TODO: Add this matrix along with the others
    elif n % 64 == 0:
        assert (is_pow2(n // 64))
        K = 64
        hadK = torch.tensor([[1 if char == '+' else -1
                              for char in line]
                             for line in hadamard_string_64.strip().split('\n')],
                            dtype=torch.float32,
                            requires_grad=False)
    elif n % 16 == 0:
        assert (is_pow2(n // 16))
        ...
        hadK = torch.tensor([[1 if char == '+' else -1
                              for char in line]
                             for line in hadamard_string_16.strip().split('\n')],
                            dtype=torch.float32,
                            requires_grad=False)
    else:
        assert (is_pow2(n))
