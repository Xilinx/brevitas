...
def _cross_layer_equalization(
    ...
    sink_weights = {name: transpose(m.weight.cpu().to(torch.float32), axis) for name, (m, axis) in sink_axes.items()}
    ...
    if merge_bias:
        src_weights = {
            name: _combine_weights_bias(transpose(m.weight, axis), bias_shrinkage, m.bias).cpu().to(torch.float32)
            for name, (m, axis) in src_axes.items()}
    else:
        src_weights = {
            name: transpose(m.weight.cpu().to(torch.float32), axis) for name, (m, axis) in src_axes.items()}
    ...
    if list_of_act_val is not None:
        ...
        srcs_range_act = scale_fn(
            torch.cat([act_val.reshape(act_val.size(0), -1).to(torch.float32) for act_val in list_of_act_val],
                      1)).cpu()
    ...
    if list_of_act_val is not None and list_of_insert_mul_node_fn is not None:
        ...
        for act_val_shape, insert_mul_node_fn in zip(list_of_act_val_shapes, list_of_insert_mul_node_fn):
            insert_mul_node_fn(inverse_scaling_factors.to(device=device, dtype=dtype), act_val_shape, act_axis)
    if len(src_axes) > 0:
