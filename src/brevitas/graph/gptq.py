...
class GPTQ():
    ...
    def get_quant_weights(self, i, i1, i2, permutation_list):
        ...
        if self.groups > 1 or (self.groups == 1 and
                               isinstance(self.layer,
                                          (qnn.QuantConvTranspose1d, qnn.QuantConvTranspose2d))):
            ...
            q = self.layer.quant_weight(subtensor_slice_list=index_2d_to_nd)
            q = q.value
            if isinstance(self.layer, SUPPORTED_CONV_OP):
                q = q.flatten(1)
            q = q.squeeze(1)
