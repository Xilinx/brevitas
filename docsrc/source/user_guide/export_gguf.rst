...
The first two modes can be obtained through our LLM entrypoint (`brevitas_ptq_llm`),
...
If activation bit-width is not specified, weight-only quantization will be performed.
...
.. code-block:: python
    ...
    class QuantZeroPointInt(Int8WeightPerTensorFloat):
        module = (this << 1).module
    ...
    class QuantScaleQuantZeroPointInt8WeightPerTensorFloat(ShiftedUint8WeightPerTensorFloat):
        proxy_class = GroupwiseWeightQuantProxyFromInjector
        scaling_int_quant = QuantScalingInt
        zp_int = QuantZeroPointInt
...
The intuition behind these quantizers is as follow:
...
These value functions pick up the object to instantiate from two other variables defined in the main quantizer, `scaling_int_quant` and `zp_int`.
...
Starting from a standard 8-bit intenger quantizer, some parameters are re-defined to match the Q4_K recipe, in particular:
